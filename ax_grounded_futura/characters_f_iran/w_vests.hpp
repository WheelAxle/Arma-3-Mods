    class AX_V_OCarrierRig_CQB_alt_hex_F: Atlas_V_OCarrierRig_CQB_alt_oli_F
	{
		author="$STR_A3_GRFU_Axle";
		scope=2;
		scopeArsenal=2;
		displayName="$STR_A3_GRFU_V_OCarrierRig_CQB_alt_hex_F0";
		picture="\ax_grounded_futura\characters_f_iran\vests\data\ui\icon_V_OCarrierRig_CQB_alt_hex_F_ca.paa";
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_hex_CO.paa",
			"\ax_grounded_futura\characters_f_iran\vests\data\CarrierRigKBT_01_Hex_CO.paa",
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_GL_hex_CO.paa"
		};
	};
	class AX_V_OCarrierRig_CQB_alt_blk_uhex_F: Atlas_V_OCarrierRig_CQB_alt_oli_F
	{
		author="$STR_A3_GRFU_Axle";
		scope=2;
		scopeArsenal=2;
		displayName="$STR_A3_GRFU_V_OCarrierRig_CQB_alt_blk_uhex_F0";
		picture="\ax_grounded_futura\characters_f_iran\vests\data\ui\icon_V_OCarrierRig_CQB_alt_blk_uhex_F_ca.paa";
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_blk_CO.paa",
			"\ax_grounded_futura\characters_f_iran\vests\data\CarrierRigKBT_01_UHex_CO.paa",
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_GL_blk_CO.paa"
		};
	};
	class AX_V_OCarrierRig_CQB_alt_khk_hex_F: Atlas_V_OCarrierRig_CQB_alt_oli_F
	{
		author="$STR_A3_GRFU_Axle";
		scope=1;
		scopeArsenal=1;
		displayName="$STR_A3_GRFU_V_OCarrierRig_CQB_alt_khk_hex_F0";
		picture="\A3_Atlas\Characters_F_Atlas\Vests\Data\UI\Atlas_V_OCarrierRig_CQB_khk_F_ca.paa";
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_khk_CO.paa",
			"\ax_grounded_futura\characters_f_iran\vests\data\CarrierRigKBT_01_Hex_CO.paa",
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_GL_khk_CO.paa"
		};
	};
	class AX_V_OCarrierRig_GL_alt_hex_F: Atlas_V_OCarrierRig_GL_alt_Oli_F
	{
		author="$STR_A3_GRFU_Axle";
		scope=2;
		scopeArsenal=2;
		displayName="$STR_A3_GRFU_V_OCarrierRig_GL_alt_hex_F0";
		picture="\ax_grounded_futura\characters_f_iran\vests\data\ui\icon_V_OCarrierRig_GL_alt_hex_F_ca.paa";
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_hex_CO.paa",
			"\ax_grounded_futura\characters_f_iran\vests\data\CarrierRigKBT_01_Hex_CO.paa",
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_GL_hex_CO.paa"
		};
	};
	class AX_V_OCarrierRig_GL_alt_blk_uhex_F: Atlas_V_OCarrierRig_GL_alt_Oli_F
	{
		author="$STR_A3_GRFU_Axle";
		scope=2;
		scopeArsenal=2;
		displayName="$STR_A3_GRFU_V_OCarrierRig_GL_alt_blk_uhex_F0";
		picture="\ax_grounded_futura\characters_f_iran\vests\data\ui\icon_V_OCarrierRig_GL_alt_blk_uhex_F_ca.paa";
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_blk_CO.paa",
			"\ax_grounded_futura\characters_f_iran\vests\data\CarrierRigKBT_01_UHex_CO.paa",
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_GL_blk_CO.paa"
		};
	};
	class AX_V_OCarrierRig_GL_alt_khk_hex_F: Atlas_V_OCarrierRig_GL_alt_Oli_F
	{
		author="$STR_A3_GRFU_Axle";
		scope=1;
		scopeArsenal=1;
		displayName="$STR_A3_GRFU_V_OCarrierRig_GL_alt_khk_hex_F0";
		picture="\A3_Atlas\Characters_F_Atlas\Vests\Data\UI\Atlas_V_OCarrierRig_GL_khk_F_ca.paa";
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_khk_CO.paa",
			"\ax_grounded_futura\characters_f_iran\vests\data\CarrierRigKBT_01_Hex_CO.paa",
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_GL_khk_CO.paa"
		};
	};
	class AX_V_OCarrierRig_Lite_alt_hex_F: Atlas_V_OCarrierRig_Lite_Alt_Oli_F
	{
		author="$STR_A3_GRFU_Axle";
		scope=2;
		scopeArsenal=2;
		displayName="$STR_A3_GRFU_V_OCarrierRig_Lite_alt_hex_F0";
		picture="\ax_grounded_futura\characters_f_iran\vests\data\ui\icon_V_OCarrierRig_alt_hex_F_ca.paa";
		hiddenSelections[]=
		{
			"camo",
			"camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_hex_CO.paa",
			"\ax_grounded_futura\characters_f_iran\vests\data\CarrierRigKBT_01_Hex_CO.paa"
		};
	};
	class AX_V_OCarrierRig_Lite_alt_blk_uhex_F: Atlas_V_OCarrierRig_Lite_Alt_Oli_F
	{
		author="$STR_A3_GRFU_Axle";
		scope=2;
		scopeArsenal=2;
		displayName="$STR_A3_GRFU_V_OCarrierRig_Lite_alt_blk_uhex_F0";
		picture="\ax_grounded_futura\characters_f_iran\vests\data\ui\icon_V_OCarrierRig_alt_blk_uhex_F_ca.paa";
		hiddenSelections[]=
		{
			"camo",
			"camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_blk_CO.paa",
			"\ax_grounded_futura\characters_f_iran\vests\data\CarrierRigKBT_01_UHex_CO.paa"
		};
	};
	class AX_V_OCarrierRig_Lite_alt_khk_hex_F: Atlas_V_OCarrierRig_Lite_Alt_Oli_F
	{
		author="$STR_A3_GRFU_Axle";
		scope=1;
		scopeArsenal=1;
		displayName="$STR_A3_GRFU_V_OCarrierRig_Lite_alt_khk_hex_F0";
		picture="\A3_Atlas\Characters_F_Atlas\Vests\Data\UI\Atlas_V_OCarrierRig_Lite_khk_F_ca.paa";
		hiddenSelections[]=
		{
			"camo",
			"camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\A3_Atlas\Characters_F_Atlas\Vests\data\OCarrierRig_khk_CO.paa",
			"\ax_grounded_futura\characters_f_iran\vests\data\CarrierRigKBT_01_Hex_CO.paa"
		};
	};
	class V_HarnessO_gry;
	class AX_V_HarnessO_hex_F: V_HarnessO_gry
	{
		author="$STR_A3_GRFU_Axle";
		displayName="$STR_A3_GRFU_V_HarnessO_hex_F0";
		picture="\A3\characters_f\Data\UI\icon_V_HarnessO_brn_CA.paa";
		hiddenSelectionsTextures[]=
		{
			"\A3\Characters_F\OPFOR\Data\clothing_CO.paa",
			"\ax_grounded_futura\characters_f_iran\headgear\data\tech_Hex2_CO.paa"
		};
	};
	class V_HarnessO_brn;
	class V_HarnessOGL_brn: V_HarnessO_brn {class ItemInfo;};
	class AX_V_HarnessOGL_hex_F: V_HarnessOGL_brn
	{
		author="$STR_A3_GRFU_Axle";
		displayName="$STR_A3_GRFU_V_HarnessOGL_hex_F0";
		picture="\A3\characters_f\Data\UI\icon_V_HarnessOGL_brn_CA.paa";
		hiddenSelections[]=
		{
			"camo1",
			"camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\A3\Characters_F\OPFOR\Data\clothing_CO.paa",
			"\ax_grounded_futura\characters_f_iran\headgear\data\tech_Hex2_CO.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo1",
				"camo2"
			};
		};
	};
	class V_HarnessOSpec_brn;
	class AX_V_HarnessOSpec_hex_F: V_HarnessOSpec_brn
	{
		author="$STR_A3_A_AveryTheKitty";
		displayName="$STR_A3_GRFU_V_HarnessOSpec_hex_F0";
		picture="\A3_Aegis\Characters_F_Aegis\Vests\Data\UI\icon_V_HarnessOSpec_brn_CA.paa";
		hiddenSelectionsTextures[]=
		{
			"\A3\Characters_F\OPFOR\Data\clothing_CO.paa",
			"\ax_grounded_futura\characters_f_iran\headgear\data\tech_Hex2_CO.paa"
		};
	};
	